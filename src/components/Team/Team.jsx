import './Team.scss'
import Marcus from '../../assets/PA-Marcus-Ohnemus.png'
import Alexander from '../../assets/PA-Sascha-Pavlovski.png'
import Dominik from '../../assets/PA-Dominik-Erb.png'

const Team = () => {
  return (
    <div className='team'>
      <span>Unser Team</span>
      <h2  data-aos="zoom-in">Über uns</h2>
      <p>Wir sind ein AI-Startup aus München, das sich darauf spezialisiert hat, mittelständische Unternehmen bei der Umsetzung der CSRD-Berichterstattung zu unterstützen.</p>

        <div className='team__members'>
            <div data-aos="fade-right" className='team__member'>
                <img src={Marcus} alt='Marcus Ohnemus' />
                <h3>Marcus Ohnemus</h3>
                <p>CEO & Founder</p>
            </div>
            <div data-aos="fade-up" className='team__member'>
                <img src={Alexander} alt='Alexander Pavlovski' />
                <h3>Alexander Pavlovski</h3>
                <p>CTO & Co-Founder</p>
            </div>
            <div data-aos="fade-left" className='team__member'>
                <img src={Dominik} alt='Dominik Erb' />
                <h3>Dominik Erb</h3>
                <p>CPO & Co-Founder</p>
            </div>
        </div>    
    </div>
  )
}

export default Team
